...
import icon from '../../../../icons/link.svg';
...
export default class ButtonUI extends Plugin {
  ...
  _createFormView(locale) {
    const editor = this.editor;
    const colorOption = editor.config._config.button.colors.options;
    const bgColorOption = editor.config._config.button.background_colors.options;
    const defaultColor = editor.config._config.button.colors.default_color;
    const defaultBgColor = editor.config._config.button.background_colors.default_background_color;
    const componentFactory = editor.ui.componentFactory;
    const formView = new FormView( locale, componentFactory, colorOption, bgColorOption, defaultColor, defaultBgColor );
  ...
  _showUI(selectedButton) {
    ...
    if (selectedButton) {
      ...
      const bg = selectedButton.getAttribute('linkButtonBgColor');
    ...
    if ( commandValue ) {
      ...
      this.formView.bgColorDropdown.fieldView.value = commandValue.bg;
    }
    setTimeout(() => {
