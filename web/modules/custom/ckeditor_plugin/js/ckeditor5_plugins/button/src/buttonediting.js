...
export default class ButtonEditing extends Plugin {
  ...
  _defineSchema() {
    ...
    schema.register('linkButton', {
      ...
      isObject: true,
      isInline: true,
      allowAttributes: ['linkButtonColor', 'linkButtonHref', 'linkButtonBgColor']
    });
  ...
  _defineConverters() {
    ...
    const colorOptions = this.editor.config._config.button.colors.options;
    const bgColorOptions = this.editor.config._config.button.background_colors.options;
    ...
    conversion.attributeToAttribute(buildAttributeToAttributeDefinition('linkButtonBgColor', bgColorOptions));
    ...
    conversion.for('upcast').add(dispatcher => {
      ...
      dispatcher.on('element:a', (evt, data, conversionApi) => {
        if (conversionApi.consumable.consume(data.viewItem, { name: true, classes: 'button-configure', attributes: ['href'] })) {
    ...
    conversion.for('upcast').elementToElement({
      ...
      view: {
        name: 'span',
        classes: 'button-configure-contents',
      }
    });
    ...
    conversion.for('editingDowncast').elementToElement({
      ...
      view: (modelElement, { writer }) =>
        toWidget(
          writer.createContainerElement('a', { class: 'button-configure', onclick: 'event.preventDefault()' }, { renderUnsafeAttributes: ['onclick'] }),
          writer, { label: 'button widget' },
        )
    });
    conversion.for('editingDowncast').elementToElement({
      model: 'linkButtonContents',
      view: (modelElement, { writer }) => toWidgetEditable(writer.createEditableElement('span', { class: 'button-configure-contents' }), writer)
    });
  }
}
